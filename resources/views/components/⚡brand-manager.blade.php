...
<div class="min-h-screen text-slate-100">
    ...
    @if(session()->has('message'))
        <div
            class="mb-6 flex items-center gap-3 p-4 bg-emerald-950/40 border border-emerald-800/60 text-emerald-300 rounded-xl backdrop-blur-md">
    ...
    @if(session()->has('error'))
        <div
            class="mb-6 flex items-center gap-3 p-4 bg-rose-950/40 border border-rose-800/60 text-rose-300 rounded-xl backdrop-blur-md">
            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="bg-slate-900/60 backdrop-blur-md rounded-2xl border border-slate-800/80 shadow-xl overflow-hidden">
        <table class="min-w-full divide-y divide-slate-800/80">
            <thead class="bg-slate-900/80">
                <tr>
                    <th class="px-6 py-4 text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider">Brand
                    </th>
                    <th class="px-6 py-4 text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider">Domain
                    </th>
                    <th class="px-6 py-4 text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider">Status
                    </th>
                    <th class="px-6 py-4 text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider">Users
                    </th>
            ...
            <tbody class="divide-y divide-slate-800/60">
                @forelse($brands as $brand)
                    <tr class="hover:bg-slate-800/30 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 text-xs font-medium rounded-full bg-sky-950/60 text-sky-300 border border-sky-800/50">
                                {{ ucfirst($brand->domain_type) }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 text-xs font-medium rounded-full {{ $brand->is_active ? 'bg-emerald-950/60 text-emerald-300 border border-emerald-800/50' : 'bg-slate-800/60 text-slate-400 border border-slate-700/50' }}">
                                <span
                                    class="w-1.5 h-1.5 rounded-full mr-1.5 {{ $brand->is_active ? 'bg-emerald-500 animate-pulse' : 'bg-slate-500' }}"></span>
    ...
    @if($showModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm"
            wire:key="brand-modal">
            <div
                class="bg-slate-900/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-800 max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                ...
                <form wire:submit="save" class="px-6 py-5 overflow-y-auto flex-1">
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            @error('websiteUrl') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select wire:model="domainType"
                                ...
                                @foreach($domainTypes as $key => $domain)
                                    <option value="{{ $key }}" class="bg-slate-900">{{ $domain['label'] }} –
                                        {{ $domain['description'] }}
                                    </option>
                                @endforeach
                            </select>
                            @error('domainType') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            @error('brandVoice') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            @error('timezone') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span>
                            @enderror
                        </div>
